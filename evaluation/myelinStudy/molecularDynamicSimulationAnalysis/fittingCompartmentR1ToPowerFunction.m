...
%% fit power function to r1_SM with different ULF values

lowFieldLimits = [0.005 0.01 0.05 0.1 0.15 0.2];

for lowFieldLimitNr = 1:length(lowFieldLimits)
    fieldStrengtIndices = r1Data.fieldStrengths > lowFieldLimits( ...
        lowFieldLimitNr);
    [factor(lowFieldLimitNr),exponent(lowFieldLimitNr) ...
        ,covariance(lowFieldLimitNr,:,:)] ...
        = fitFieldStrengthBehaviorOfR1ToPowerFunctionWithoutWeights( ...
        r1Data.fieldStrengths(fieldStrengtIndices) ...
        ,r1Data.r1Eff_SM(fieldStrengtIndices) ...
        ,[0.5 0.5],varargin); %#ok<SAGROW>
    fprintf("  %.7f %.7f \n",covariance(lowFieldLimitNr,:,:));
    fprintf(" \n\n");
